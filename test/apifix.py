from fastapi import FastAPI, HTTPException, responses, File
import csv
import json
import zipfile
...
app = FastAPI()
@app.get("/map/{file_id}")
async def get_csv_files(file_id: str):
    ...
    return StreamingResponse(generate_zip(), media_type="application/zip", headers={"Content-Disposition": f'attachment; filename="combined_files.zip"'})